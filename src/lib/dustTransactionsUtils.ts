...
export class DustTransactionsUtils {
    ...
    static async buildRegistrationTransaction(lucid: LucidEvolution, contracts: ContractsRegistry, dustPKH: string): Promise<TxSignBuilder> {
        ...
        console.log('[DustTransactions]', '🔧 Building DUST Address Registration Transaction...');

        // Get contracts from registry
        console.log('[DustTransactions]', '🔍 Getting contracts from registry...');
        ...
        if (!versionOracleValidatorContract || !dustAuthTokenMintingPolicyContract || !dustAuthTokenPolicyContract || !dustMappingValidatorContract) {
            console.error('[DustTransactions]', '❌ Required contracts not found in registry');
            throw new Error('Required contracts not found in registry');
        }

        // Find Version Oracle UTxO (reference input)
        console.log('[DustTransactions]', '🔍 Querying Version Oracle UTxOs for reference...');
        ...
        if (versionOracleUtxos.length === 0) {
            console.error('[DustTransactions]', '❌ Version Oracle UTxOs not found');
        ...
        if (!dustAuthTokenMintingUtxo) {
            console.error('[DustTransactions]', '❌ DUST Auth Token Minting Policy UTxO not found');
        ...
        if (!dustPKH) {
            console.error('[DustTransactions]', '❌ DUST PKH not configured');
        ...
        console.log('[DustTransactions]', '🔨 Building registration transaction...');
        const txBuilder = lucid.newTx();

        // REFERENCE INPUT: Version Oracle UTxO with DUST Auth Token Minting Policy
        console.log('[DustTransactions]', '📥 Adding reference input (Version Oracle UTxO with minting policy)...');
        ...
        console.log('[DustTransactions]', '📎 Attaching DUST Auth Token Minting Policy script...');
        txBuilder.attach.MintingPolicy(dustAuthTokenMintingPolicyContract.scriptObject!);

        console.log('[DustTransactions]', '📎 Attaching DUST Auth Token Policy script...');
        txBuilder.attach.MintingPolicy(dustAuthTokenPolicyContract.scriptObject!);

        // Add signer
        txBuilder.addSigner(await lucid.wallet().address());

        console.log('[DustTransactions]', '🔧 Completing registration transaction...');
        const completedTx = await txBuilder.complete();

        console.log('[DustTransactions]', '✅ Registration transaction completed successfully');
        return completedTx;
    }
    ...
    static createRegistrationExecutor(lucid: LucidEvolution, contracts: ContractsRegistry, dustPKH: string) {
        return async (params: Record<string, unknown>, onProgress?: (step: string, progress: number) => void): Promise<string> => {
            ...
            console.log('[DustTransactions]', '✍️ Signing registration transaction...');
            const signedTx = await completedTx.sign.withWallet().complete();

            onProgress?.('Submitting registration transaction...', 60);
            console.log('[DustTransactions]', '📤 Submitting registration transaction...');
            const txHash = await signedTx.submit();

            console.log('[DustTransactions]', '🎯 Registration transaction submitted successfully:', txHash);
            return txHash;
        };
    }
    ...
    static async buildUnregistrationTransaction(lucid: LucidEvolution, contracts: ContractsRegistry, dustPKH: string, registrationUtxo: UTxO): Promise<TxSignBuilder> {
        ...
        console.log('[DustTransactions]', '🔧 Building DUST Address Unregistration Transaction...');
        ...
        console.log('[DustTransactions]', '🔍 Querying Version Oracle UTxOs for reference inputs...');
        const versionOracleUtxos = await lucid.utxosAt(versionOracleValidatorContract.address!);
        ...
        if (!dustAuthTokenBurningUtxo) {
            console.error('[DustTransactions]', '❌ DUST Auth Token Burning Policy UTxO not found');
        ...
        if (!dustMappingValidatorSpendUtxo) {
            console.error('[DustTransactions]', '❌ DUST Mapping Validator Spend Policy UTxO not found');
            throw new Error('DUST Mapping Validator Spend Policy UTxO not found');
        }

        // Build the unregistration transaction
        console.log('[DustTransactions]', '🔨 Building unregistration transaction...');
        const txBuilder = lucid.newTx();

        // REFERENCE INPUTS: Version Oracle UTxOs with required policies
        console.log('[DustTransactions]', '📥 Adding reference inputs (Version Oracle UTxOs with policies)...');
        ...
        console.log('[DustTransactions]', '📎 Attaching required scripts...');
        ...
        console.log('[DustTransactions]', '🔧 Completing unregistration transaction...');
        const completedTx = await txBuilder.complete();

        console.log('[DustTransactions]', '✅ Unregistration transaction completed successfully');
        return completedTx;
    }
    ...
    static createUnregistrationExecutor(lucid: LucidEvolution, contracts: ContractsRegistry, dustPKH: string, registrationUtxo: UTxO) {
        return async (params: Record<string, unknown>, onProgress?: (step: string, progress: number) => void): Promise<string> => {
            ...
            console.log('[DustTransactions]', '✍️ Signing unregistration transaction...');
            const signedTx = await completedTx.sign.withWallet().complete();

            onProgress?.('Submitting unregistration transaction...', 60);
            console.log('[DustTransactions]', '📤 Submitting unregistration transaction...');
            const txHash = await signedTx.submit();

            console.log('[DustTransactions]', '🎯 Unregistration transaction submitted successfully:', txHash);
            return txHash;
        };
    }
    ...
    static async buildUpdateTransaction(lucid: LucidEvolution, contracts: ContractsRegistry, newDustPKH: string, registrationUtxo: UTxO): Promise<TxSignBuilder> {
        ...
        console.log('[DustTransactions]', '🔧 Building DUST Address Update Transaction...');
        ...
        if (!newDustPKH) {
            console.error('[DustTransactions]', '❌ New DUST PKH not provided');
        ...
        console.log('[DustTransactions]', '🔨 Building update transaction...');
        const txBuilder = lucid.newTx();

        // REFERENCE INPUT: Version Oracle UTxO with required policy
        console.log('[DustTransactions]', '📥 Adding reference input (Version Oracle UTxO with spend policy)...');
        ...
        console.log('[DustTransactions]', '🔧 Completing update transaction...');
        const completedTx = await txBuilder.complete();

        console.log('[DustTransactions]', '✅ Update transaction completed successfully');
        return completedTx;
    }
    ...
    static createUpdateExecutor(lucid: LucidEvolution, contracts: ContractsRegistry, newDustPKH: string, registrationUtxo: UTxO) {
        return async (params: Record<string, unknown>, onProgress?: (step: string, progress: number) => void): Promise<string> => {
            ...
            console.log('[DustTransactions]', '✍️ Signing update transaction...');
            const signedTx = await completedTx.sign.withWallet().complete();

            onProgress?.('Submitting update transaction...', 60);
            console.log('[DustTransactions]', '📤 Submitting update transaction...');
            const txHash = await signedTx.submit();

            console.log('[DustTransactions]', '🎯 Update transaction submitted successfully:', txHash);
            return txHash;
        };
    }
}
